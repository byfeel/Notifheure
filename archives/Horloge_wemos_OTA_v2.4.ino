...
void handleSubmit() {
  ...
  if ( LUM =="manu" ) {
            AutoIn=false;
            sendToJeedom(BaseUrlJeedom , idAuto ,"manu");
            if (server.arg("INT") !="") Intensite=server.arg("INT").toInt();
            sendToJeedom(BaseUrlJeedom , idLum , String(Intensite));
    } else if ( LUM == "auto") 
        { 
          AutoIn=true;
          sendToJeedom(BaseUrlJeedom , idAuto ,"auto");
          }
  ...
  if ( SEC == "on" ) {
    DisSec=true;
    sendToJeedom(BaseUrlJeedom , idSec,"on");
    server.send ( 200, "text/html", getPage() );
  } else if ( SEC == "off" ) {
    DisSec=false;
     sendToJeedom(BaseUrlJeedom , idSec,"off");
    server.send ( 200, "text/html", getPage() );
  } else {
   server.send ( 200, "text/html", getPage() );
  }
...
if (  DisSec ) sendToJeedom(BaseUrlJeedom , idSec,"on");
   ...
   if ( AutoIn ) sendToJeedom(BaseUrlJeedom , idAuto,"auto");
  else sendToJeedom(BaseUrlJeedom , idAuto,"manu");

   if (TimeOn) sendToJeedom(BaseUrlJeedom , idEtatHorloge,"on");
  else sendToJeedom(BaseUrlJeedom ,idEtatHorloge,"off");
...
// Gestion Affichage
  ...
  // Sinon affichage de l'heure
      ...
      if (TimeOn) {
       ...
       //P.print(httpCode); //Debug pour afficher valeur 
